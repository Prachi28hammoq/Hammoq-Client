...
export default class AdvanceListing extends React.Component {
  ...
  Axios.get(url, headers)
       .then((response) => {
              ...
              let productData;
              if(isTemplate) productData=response.data.templates[0].data;
              else productData = response.data.products[0];
              try
              {
  ...
  onSubmit(e,  queuePlacement, action) {
    e.preventDefault();
    const { data, images, clientid, productid,isTemplate } = this.state;
        ...
        var imagedata = new FormData();
              ...
              await Promise.all(images.filter(image => !!image.img).map(async(image) => {if(!!image.img)
              {
                if(typeof image.img === 'object')
                {
                  ...
                  let response = await Axios.put("/product/images", imagedata, {headers: {"Content-Type": "multipart/form-data"}}).catch((err) => {this.setState({ isSubmitting: false });console.log(err);});
  ...
  toggleSelectedOthersWebsite(index)
  {
    const { data } = this.state;
    data.others[index].status = !data.others[index].status;
    this.setState({ data });
  };
